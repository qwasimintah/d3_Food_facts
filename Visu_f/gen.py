...
N = len(COLUMNS)
CODE, PRODUCT, CAT, NADDS, ADDS, IMAGE, GRADE = (COLUMNS.index(col) for col in ["code", "product_name", "categories_tags", 'additives_n', 'additives_tags', 'image_small_url', 'nutrition_grade_fr'])
...
products.write("%s\n" % '\t'.join(["code", "name", "image", "n_additives", "nutriscore", *INGREDIENTS]))
...
for line in offp:
	...
	grade = words[GRADE]
	grades[grade] += 1
	products.write("%s\n" % '\t'.join([code, name, image, str(n_additives), grade, *(words[i].strip() for i in INGREDIENTS_COLS)]))
